...
- [x] **Git Manager** — bar chip (Git logo SVG via `document.createElementNS`) with red dot when failed CI > 0; click opens acrylic tabbed window (Failed CI / Open PRs / Overview) with account filter pills; providers: GitHub (GraphQL), GitLab (REST v4, cloud + self-hosted), Bitbucket Cloud (REST 2.0), Bitbucket Server (REST 1.0), Forgejo/Gitea (REST v1); accounts stored with DPAPI-protected tokens; background 30s poll thread with immediate fetch on config save; Forgejo & Gitea providers share implementation
